...
public class Spawner : MonoBehaviour
{
    ...
    public float yOffset = 0.5f;      // Altura por encima del suelo
    ...
    void SpawnCreature()
    {
        ...
        PokemonData selectedPokemon = GetRandomPokemon();
        if (selectedPokemon == null) return;

        GameObject creatureInstance = Instantiate(selectedPokemon.prefab, spawnPosition, Quaternion.identity);
        CreatureBehavior creatureBehavior = creatureInstance.GetComponent<CreatureBehavior>();
        if (creatureBehavior == null)
        {
            creatureBehavior = creatureInstance.AddComponent<CreatureBehavior>();
        }

        // Selecciona el nivel en 1 Rango entre 2 niveles Min y Max
        int level = Random.Range(nivelMin, nivelMax + 1);

        // Se selecciona el sexo. Revisar ya que depende del PokemonData la proporcion de aparición
        Gender gender;
        if (selectedPokemon.genderType == Gender.Unknown)
        {
            gender = Gender.Unknown;
        }
        else
        {
            gender = (Random.value < selectedPokemon.maleRatio) ? Gender.Male : Gender.Female;
        }

        // Calculamos si es shiny
        bool isShiny = Random.value < GameManager.Instance.shinyProbability;

        //Instanciamos el pokemon con los atributos definidos:
        PokemonInstance instance = new PokemonInstance(selectedPokemon, level, gender, isShiny);
        ...
        {
            creatureBehavior.player = playerObject.transform;
        }
