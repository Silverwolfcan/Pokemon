...
public class TurnController : MonoBehaviour
{
    ...
    // Estados (se buscan en los TRANSFORMS reales)
    ...
    public IEnumerator DoPlayerTurn(Vector3 ringCenter, float combatantOffsetFromCenter)
    {
        ...
        if (queuedUseItem.HasValue) { var (item, target) = queuedUseItem.Value; yield return DoUseItem(item, target); ClearQueue(); playerStatus?.OnEndOfTurn(); IsResolving = false; yield break; }

        if (queuedMoveIndex.HasValue)
        {
            yield return ExecuteMove(player, enemy, queuedMoveIndex.Value);
            ClearQueue();
        }
    ...
    public IEnumerator DoEnemyTurn(Vector3 ringCenter, float combatantOffsetFromCenter)
    {
        ...
        int idx = FindUsableMoveIndex(enemy?.Model, -1);
        if (idx >= 0) yield return ExecuteMove(enemy, player, idx);
        else yield return new WaitForSeconds(0.25f);

        // Cambio si el jugador muere
        if (player?.Model != null && player.Model.currentHP <= 0)
        {
            var partyObj = PokemonStorageManager.Instance?.PlayerParty; // tipo agnóstico
            if (HasAliveReplacement(partyObj, player.Model))
    ...
    private IEnumerator DoUseItem(ItemData item, PokemonInstance target)
    {
        if (item == null || target == null) yield break;
        Debug.Log($"[Bag] Usar {item.name} sobre {target.DisplayName}");
        yield return new WaitForSeconds(0.2f);
    }

    private IEnumerator ExecuteMove(CombatantController attacker, CombatantController defender, int moveIndex)
    {
        var atkMon = attacker?.Model;
        var defMon = defender?.Model;
        if (atkMon == null || defMon == null) yield break;

        var moves = atkMon.Moves;
        if (moveIndex < 0 || moveIndex >= moves.Count) yield break;
        var inst = moves[moveIndex];
        var data = inst?.data;
        ...
        if (hit)
        {
            int dmg = MoveExecutor.ComputeDamage(atkMon, defMon, data);

            // Penalización por quemadura en ataques físicos
            if (data.attackCategory == AttackType.Physical)
            {
                var atkStatus = attacker == player ? playerStatus : enemyStatus;
                if (atkStatus != null)
                    dmg = Mathf.FloorToInt(dmg * StatusService.GetAttackMulByStatus(atkStatus.Primary));
            }

            if (dmg > 0)
            {
                defMon.currentHP = Mathf.Max(0, defMon.currentHP - dmg);
                Debug.Log($"[Damage] {defMon.DisplayName} -{dmg} ({defMon.currentHP}/{defMon.stats.MaxHP})");
            }

            MoveExecutor.ApplySecondaryEffects(atkMon, defMon, data);
        }
    ...
    private int FindUsableMoveIndex(PokemonInstance mon, int preferredIndex)
    {
        ...
        var list = mon.Moves;
        if (preferredIndex >= 0 && preferredIndex < list.Count && list[preferredIndex] != null && list[preferredIndex].currentPP > 0)
            return preferredIndex;

        for (int i = 0; i < list.Count; i++)
            if (list[i] != null && list[i].currentPP > 0) return i;
        return -1;
    }
    ...
    private static bool HasAliveReplacement(object partyObj, PokemonInstance current)
    {
        ...
        // 1) Si el objeto es IEnumerable, itera directamente.
        if (partyObj is System.Collections.IEnumerable en)
        {
            foreach (var it in en)
                if (it is PokemonInstance p && p != current && p.currentHP > 0) return true;
        }

        // 2) Intenta campos/propiedades comunes por reflexión.
